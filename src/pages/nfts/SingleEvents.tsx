...
function SingleEventPage() {
  ...
  useEffect(() => {
    async function fetchEvent() {
      try {
        const lang = i18n.language || "en"; // ✅ fallback if missing
        const res = await fetch(
          `${API_BACKEND_URL}/eventById/${id}?page=2&lang=${lang}` // ✅ added lang param
        );
  ...
  if (!event)
    return <div style={{ color: "#fff" }}>Event not found.</div>;
